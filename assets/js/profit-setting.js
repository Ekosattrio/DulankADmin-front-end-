document.addEventListener('DOMContentLoaded', () => {
	...
	function checkValue() {
		...
		const forms = [
			...
			scope.querySelector('.pajak-form')
		].filter(Boolean);
	...
	function createCostEditRow(label = '', unit = 'Per Pekerjaan/Job', value = '', cancellable = false) {
		...
		row.innerHTML = `
			...
			<div class="d-flex mt-3 col-md-3">
				<button class="btn btn-link btn-selesai" type="button">Selesai</button>
				${cancellable ? '<button class="btn btn-link text-danger text-decoration-none btn-batal" type="button">Batal</button>' : ''}
			</div>
		`;
	...
	function attachCostHandlers(row) {
		...
		row.querySelector('.btn-edit')?.addEventListener('click', () => {
			...
			const unit = row.querySelector('.price-input')?.nextElementSibling?.nextElementSibling?.textContent.trim()
	...
	scope.querySelector('.range-form')?.addEventListener('focusout', event => {
		...
		const keValue = parseInt((event.target.parentElement.querySelector('.result-input')?.value || event.target.value).replace(/\D/g, ''), 10);
