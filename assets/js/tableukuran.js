...
document.getElementById('formEditRow').onsubmit = function(e) {
    ...
    bootstrap.Modal.getInstance(document.getElementById('modalEditUkuran')).hide();
								...
								// Hapus & Edit
								document.querySelector('#tableSorting tbody').onclick = function(e) {
									const tr = e.target.closest('tr');
									if(e.target.closest('.btn-delete-sorting')) {
										tr.remove();
										// Update nomor urut
										document.querySelectorAll('#tableSorting tbody tr').forEach((row, i) => {
											row.children[0].innerText = i+1;
										});
									}
									if(e.target.closest('.btn-edit-sorting')) {
										const idx = Array.from(tr.parentNode.children).indexOf(tr);
										document.getElementById("modalSortingLabel").innerText = "Edit Sorting Tag";
										document.getElementById("inputSortingNama").value = tr.children[1].innerText;
										document.getElementById("inputSortingQty").value = tr.children[2].innerText;
										document.getElementById("inputSortingUkuran").value = tr.children[3].innerText;
										document.getElementById("inputSortingKertas").value = tr.children[4].innerText;
										document.getElementById("inputSortingSisi").value = tr.children[5].innerText;
										document.getElementById("inputSortingLaminasi").value = tr.children[6].innerText;
										document.getElementById("inputSortingLipat").value = tr.children[7].innerText;
										document.getElementById("inputSortingRoute").value = tr.children[8].innerText;
										document.getElementById("inputSortingStatus").value = tr.children[9].innerText.trim();
										document.getElementById("editSortingIndex").value = idx;
										new bootstrap.Modal(document.getElementById('modalSorting')).show();
									}
								};
